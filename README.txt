...
3. 水印模板支持“左下角经典模板”和“居中简洁模板”两个版本。
4. 自动识别地址会优先读取照片 EXIF GPS；如果没有 GPS，会尝试在浏览器本地 OCR 识别图片中文字，识别不到就不修改地点。
5. 右下角“水印相机”图标使用 camera-icon-template.png，必须和 index.html 放在同一目录。
6. 如果手机打开后仍显示旧图标，请刷新页面，或清除浏览器缓存后重新打开。
